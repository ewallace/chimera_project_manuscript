...
The mTurquoise2 coding sequence is as used in @Lee2015moclo. 
...
RT-qPCR data was analysed using our tidyqpcr R package [@Wallace2022].
...
Alternative Poly(A) site usage was tested using a Mann–Whitney U test [@mann1947] on relative reads mapped to a 9 nucleotide window centred on the major Poly(A) site.
Then, p-values were calculated by comparing construct relative counts to wild-type relative counts and adjusted p-values were calculated using the false discovery rate [@benjamini1995].
...
Then, p-values were calculated using t-tests and adjusted p-values calculated using the false discovery rate [@benjamini1995].
...
Further modified 3'UTR-terminator designs were designed to incorporate individual motifs of interest previously identified, within the insertion sites described.
...
Similar to the modified tRPS3 designs, in tTSA1 we designed three 9 nt motif insertion sites: M1 21 nt downstream of the stop codon, M2 20 nt downstream of M1, and M3 24 bp downstream of M2.
...
The PIR1 terminator chosen for motif deletion contains one copy each of the ATATTC and TGTAHMNT motifs, and 3 copies of the HWNCATTWY motifs, although did not contain the putative stability motif GTATACCTA.
...
Again, p-values were calculated using t-tests and adjusted p-values calculated using the false discovery rate [@benjamini1995].
...
All analysis made extensive use of the tidyverse and ggplot2 [@Wickham2019; @Wickham2016], within the R statistical programming environment [@R].
