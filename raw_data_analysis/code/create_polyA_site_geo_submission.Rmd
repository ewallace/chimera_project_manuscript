...
author: "Sam Haynes & Edward Wallace"
date: "03/11/2023"
...
Creates summary data on poly(A)-site counts from QuantSeq and 5PSeq data, to be included in summary data for GEO submission.
...
sample_to_construct_dictionary_QuantSeq_batch2 <- read_csv(here("raw_data_analysis/data/rna_seq/sample_to_construct_table_QuantSeq_batch2.csv"))

sample_to_construct_dictionary_5PSeq <- read_csv(here("raw_data_analysis/data/rna_seq/sample_to_construct_table_5PSeq.csv"))
```

# Define functions for loading poly(A)-proximal reads
...
extract_gene_counts_from_featureCounts_file <- function(filepath){
  read_tsv(filepath, comment = "#") %>%
    # select only Geneid and sample
    select(-Chr, -Start, -End, -Strand, -Length) %>%
    # rename plasmid genes
    mutate(Geneid = str_replace(Geneid, 
                                "([\\w-]+)_(primary_transcript|URA3)", 
                                "plasmid_\\2")) %>%
    # remove bits except sample name
    rename_with(~str_remove(., "_aln.bam"))
}
...
# Calculate cumulative distributions of poly(A) site positions
...
empty_full_length_QuantSeq_batch2_plasmids <- tibble(construct_name = rep(full_QuantSeq_batch2_polyA_reads_from_stop_codon %>% pull(construct_name) %>% unique(), each = 501), 
...
full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads <- full_QuantSeq_batch2_polyA_reads_from_stop_codon %>%
  ...
  inner_join(sample_to_construct_dictionary_QuantSeq_batch2, by = c("sample" = "sample_name"))  |>
...
full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads_control <-
  full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads |>
  filter(mod == "genomic") %>%
...
full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads_plasmid <-
  full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads |>
  filter(mod == "WT", !URA3) |>
  ungroup() |>
...
# Load genome-wide counts per gene
...
raw_featureCounts_files_QuantSeq_batch2 <- list.files("../data/rna_seq/counts/QuantSeq_batch2", pattern = "*.txt$", recursive = TRUE, full.names = TRUE)
...
all_sample_counts_QuantSeq_batch2 <- raw_featureCounts_files_QuantSeq_batch2 %>%
  map(extract_gene_counts_from_featureCounts_file) %>%
  reduce(full_join, by = "Geneid") %>%
  pivot_longer(-Geneid, values_to = "counts", names_to = "sample") %>%
  mutate(sample = factor(sample, levels = expand.grid("WD_C",7:16) %>% 
  ...
  mutate(promoter = str_extract(construct_name, "^p[A-Z0-9]+"), terminator = str_extract(construct_name, "(?<=\\-)t[A-Z0-9]+(?=_)")) %>%
  select(-construct_name) %>%
  unite("sample", promoter, terminator, rep) %>%
  # mutate(sample = str_replace(sample, "NA_NA_Control", "POT1_ccdB")) %>%
  pivot_wider(names_from = "sample", values_from = "counts")
...
# Write counts per gene

```{r write_out_3UTR_counts}
...
write_tsv(full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads_plasmid, here("raw_data_analysis/data/rna_seq/construct_polyA_bam/QuantSeq_batch2/QS_batch2_processed_polyA_constructs.tsv"))

write_tsv(full_length_QuantSeq_batch2_plasmids_cumulative_polyA_reads_control, here("raw_data_analysis/data/rna_seq/construct_polyA_bam/QuantSeq_batch2/QS_batch2_processed_polyA_controls.tsv"))
...
write_tsv(all_sample_counts_QuantSeq_batch2, here("raw_data_analysis/data/rna_seq/counts/QuantSeq_batch2/QS_batch2_all_sample_gene_counts.tsv"))
