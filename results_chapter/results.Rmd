...
Read 1 allows precise inference of poly(A) site position while read 2 generally overlaps the CDS and allows distinguishing terminators in native loci from reporter constructs, as long as the poly(A)-site is within about 300nt of the stop codon.
...
Poly(A) sites are largely in the same positions relative to the stop codon in wild-type terminators at native genomic loci and plasmid constructs (Figure \@ref(fig:rnaseq-QC-genomic-vs-plasmid-polyA)).
For terminators tPGK1 (expressed in pRPS3-mCherry-tTSA1), and tTSA1, tRPS3, tHSP26 and tTOS6 expressed from their cognate promoter, the major poly(A)-site is conserved in the plasmid and in most cases the poly(A)-site distribution is indistinguishable.
The tHSP26 terminator has similar poly(A)-site distribution expressed on a plasmid from pHSP26, pRPS3, and pRPS13, although there is some termination after the major poly(A)-site in pHSP26-mCherry-tHSP26.
We excluded a 420nt tPMA1 terminator construct from our analysis because this was beyond the library read length and so we could not reliably distinguish plasmid from genomic poly(A)-site reads.


```{r rnaseq-QC-genomic-vs-plasmid-polyA, fig.cap="\\textbf{Poly(A) site usage remains the same for genomic terminators as for plasmid expressed WT terminators.} Cumulative fraction of poly(A)-site reads mapped downstream of stop codons for native genomic terminators and plasmid construct terminators. Each line plots the cumulative counts of a separate replicate. In some panels the dashed line indicating plasmid-mapped reads overlaps the genome-mapped reads as the cumulative distributions are not distinguishable.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/plasmid_vs_genome_polyA_combined_plot.png"))
```


We next mapped poly(A)-sites for three motif insertion constructs with large effect sizes in the qPCR results: mod_NAA, mod_HTH and mod_NTN, together with WT and mod_NNN controls, within three promoter-terminator contexts: pRPS3-tRPS3, pPGK1-tRPS3 and pTSA1-tTSA1.
