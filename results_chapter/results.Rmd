...
Read 1 allows precise inference of poly(A) site position while read 2 generally overlaps the CDS and allows distinguishing terminators in native loci from reporter constructs, as long as the distance between poly(A)-site and stop codon is under about 300nt and so within the sequenced fragment length.
...
Poly(A) sites are in the same positions relative to the stop codon in wild-type terminators at native genomic loci and in plasmid constructs (Figure \@ref(fig:rnaseq-genomic-vs-plasmid-polyA)).
For plasmid-expressed terminators tPGK1, tTSA1, tRPS3, tHSP26 and tTOS6 driving mCherry expression, the major poly(A)-site is conserved and the poly(A)-site distribution is largely indistinguishable.
...
The tHSP26 terminator has similar poly(A)-site distribution expressed paired with pHSP26, pRPS3, and pRPS13, although there is some termination after the major poly(A)-site in pHSP26-mCherry-tHSP26.
We excluded a tPMA1 terminator from our analysis because the 420nt terminator was longer than library fragment lengths so we could not distinguish plasmid from genomic poly(A)-site reads.


```{r rnaseq-genomic-vs-plasmid-polyA, fig.cap="\\textbf{Poly(A) sites are largely in the same positions in wild-type terminators at native genomic loci and plasmid constructs.} Cumulative fraction of poly(A)-site reads mapped downstream of stop codons, for native genomic terminators and selected plasmid construct terminators. Each line plots the cumulative counts of a separate replicate, with solid line for genomic copy and dashed line for plasmid-expressed copy, coloured by promoter. In some panels the dashed line indicating plasmid-mapped reads overlaps the genome-mapped reads as the cumulative distributions are not distinguishable.", fig.align='center'}
