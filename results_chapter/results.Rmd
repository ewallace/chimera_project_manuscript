...
# NOTE: Currently this figure is included at 80% size so we can see the caption
...
We also confirmed that most differences in protein outputs are accounted for by changes in mRNA abundance for well-expressed promoters, by checking a subset of mCherry constructs using RT-qPCR ($R > 0.9$, Figure Supplementary Figure \@ref(fig:raw-pro-ter-swap-protein-fluo)BC).
...
The interaction of coding sequence and terminator is seen most clearly for tRPS3 compared to tPGK1.
Constructs with tRPS3 are consistently more highly expressed in mCherry constructs compared to mTurquoise2 constructs with the same promoter.
...
Next, we investigated the regulatory effects of short CREs, i.e. sequence motifs,  contained within terminators or 3'UTRs.
First, 69 suitable motifs to test for context dependence were found through a literature search. 
...
These datasets are loosely correlated in their half-life measurements across 4188 genes reported in both datasets, $R = 0.63$ (Figure \@ref(fig:hlife-decay-model)A). 
However, Chan et al. @Chan2018 estimated substantially shorter half-lives, had greater coverage of genes in the yeast genome (5529 vs 4304),  and used multiple time points to determine half-lives. 
...
The linear model performed similarly on both datasets by explaining 44% and 41% of the variability in half-lives for Chan and Sun datasets respectively (Figure \@ref(fig:hlife-decay-model)C).
...
The top 7 most significant motifs from the Chan data showed similar regulatory behaviour when tested on their own in the Sun data, except for TGTAAATA which was stabilising in one dataset and destabilising in another, as we later discuss (Figure \@ref(fig:hlife-decay-model)B).
...
However, comparison of the wild-type and control (mod_NNN) terminators does show that the creation of the insertion sites alone has an effect on mRNA levels (Supplementary Table \@ref(tab:insertion-construct-ttest)).
...
Decay motifs generally lead to lower mRNA abundance, although ATATTC (mod_NAA) has a weaker effect in tTSA1 than in tRPS3, and TGTAHMNTA (mod_NTN) has a stronger effect in tTSA1 than tRPS3.
...
We compared the effects of cis-regulatory motifs on mRNA abundance to predicted effects inferred from transcriptome-wide measurements of half-life.
...
Therefore, we compared the motif's estimated change in log2 mRNA abundance to that predicted due to changes in log2 half-life, estimated from the Chan et al. @Chan2018 dataset.
...
Comparing the estimated coefficients for each motif's effect on mRNA abundance across promoter-terminator pairing shows that the effect of a motif depends on terminator context (Figure \@ref(fig:hlife-predict-vs-abundance-and-motif-context-dependence)B).
...
We included an interaction term that quantifies how the effect of including both HWNCATTWY and TGTAHMNTA together differs from the sum of the effects of including these motifs individually (Figure \@ref(fig:hlife-predict-vs-abundance-and-motif-context-dependence)B).
...
```{r hlife-predict-vs-abundance-and-motif-context-dependence, fig.cap="\\textbf{Promoter and terminator context alter the regulatory effect of motifs.} (\\textbf{A}) Predicted change in transcript abundance inferred from transcriptome-wide motif contributions to half-life, compared to RT-qPCR measurements of reporter transcript abundance. The y-axis shows statistical summaries (mean and standard error) of 6 replicates for data shown in figures 4 and 5. The fold change is relative to the mod\\_NNN construct in each promoter-terminator pairing for the tRPS3 and tTSA1 sets, and relative to the wild-type constructs for tPIR1 sets. Native promoter panels show the promoter paired with the terminator from the same set, e.g. pRPS3 with tRPS3. (\\textbf{B}) Motif contributions to fold changes in mRNA abundance for reporter constructs with different promoter and terminator contexts. This is calculated by a linear model with a coefficient for the effect of each motif in each set, applied to $\\Delta Cq$ against 3 reference genes. The last column shows the interaction term between TGTAHMNTA and HWNCATTWY.", fig.align='center',out.width="85%"}
knitr::include_graphics(here("results_chapter/figures/qPCR_model_coef_and_pred_vs_exp_abund.png"))
```

## Polyadenylation site usage depends on the terminator sequence and can be changed by motif insertion

We next mapped poly(A) site usage, across a subset of reporter strains, for 3 reasons.
First, to ask if poly(A) site depends only on the terminator sequence or on the wider promoter, coding sequence, and genomic context.
Second, to find if motif effects on mRNA abundance could be due to changes in the poly(A) site usage.
Third, to check if motifs inserted in our terminators were unintentionally absent from the mature mRNA.
We performed paired-end RNA-Seq using the QuantSeq REV protocol [@Moll2014], and mapped the reads to genome sequences extended by the relevant reporter plasmid sequence (see methods).
Read 1 allows precise inference of poly(A) site position while read 2 generally overlaps the CDS and allows distinguishing terminators in native loci from reporter constructs, as long as the distance between poly(A)-site and stop codon is under about 300nt and so within the sequenced fragment length (Figure \@ref(fig:rnaseq-genomic-vs-plasmid-polyA)A)
We detected 1000s of reads on each reporter construct, which is enough to quantify poly(A) sites and expression with confidence.
We display the poly(A) sites as the cumulative fraction of poly(A)-site reads mapped at each location downstream of construct stop codons, out of all reads mapped to the terminator.
In native genes the poly(A) site locations were highly reproducible across samples (Supplementary Figure \@ref(fig:rnaseq-QC-QuantSeq-and-5PSeq-genomic-polyA)), and counts of native mRNAs are highly correlated between samples (Supplementary Figure \@ref(fig:rnaseq-QC-correlation)), giving us confidence that changes in construct detection and poly(A) sites are meaningful.

Poly(A) sites are generally in the same positions relative to the stop codon in wild-type terminators at native genomic loci and in plasmid constructs (Figure \@ref(fig:rnaseq-genomic-vs-plasmid-polyA)B).
For plasmid-expressed terminators tTSA1, tRPS3, tHSP26 and tTOS6 driving mCherry expression, the major poly(A)-site is conserved and the poly(A)-site distribution is largely indistinguishable.
Poly(A) sites are conserved in terminators regardless of the paired promoter, across tPGK1, tRPS3, and tTOS6 expressed downstream of pRPS3, and tTSA1 expressed paired with cognate pTSA1.
The tHSP26 terminator has similar poly(A)-site distribution expressed paired with pHSP26, pRPS3, and pRPS13, although there is some termination downstream of the major poly(A)-site in pHSP26-mCherry-tHSP26.
The exception is tPMA1, where the region of poly(A)-sites 350-450nt downstream of the stop codon made the observed terminator longer than RNA-seq library fragment lengths, so we could not reliably distinguish plasmid from genomic reads.
This highlights a limitation of our experimental design, and investigation of longer terminators requires either different sequencing methods or strain construction, such as replacing the promoter on the native gene.
However, having validated the assay on shorter tTSA1 and tRPS3 constructs, we next investigated the effect of terminator mutations.



```{r rnaseq-genomic-vs-plasmid-polyA, fig.cap="\\textbf{Poly(A) sites are largely in the same positions in wild-type terminators at native genomic loci and plasmid constructs.} A, Schematic of experimental design to use poly(A)-proximal RNA-seq with QuantSeq REV to distinguish poly(A)-sites in native and reporter genes. B, Cumulative fraction of poly(A)-site reads mapped downstream of stop codons, for native genomic terminators and selected plasmid construct terminators. Each line plots the cumulative counts of a separate replicate, with solid line for genomic copy and dashed line for plasmid-expressed copy, coloured by promoter. In some panels the dashed line indicating plasmid-mapped reads overlaps the genome-mapped reads as the cumulative distributions are not distinguishable.", fig.align='center'}
knitr::include_graphics(here("results_chapter/figures/plasmid_vs_genome_polyA_combined_plot.png"))
```


We then mapped poly(A)-sites for three motif insertion constructs with large effect sizes in the qPCR results: mod_NAA, mod_HTH and mod_NTN, together with wild-type and mod_NNN controls, within three promoter-terminator contexts: pRPS3-tRPS3, pPGK1-tRPS3 and pTSA1-tTSA1.
Transcript abundance, relative to mod_NNN, for most constructs correlates strongly with qPCR results and so confirms them (Figure \@ref(fig:quantseq-polyA-site-usage)A).
However, some constructs were detected as differentially abundant by RNA-Seq for reasons that are unclear.
For example, pPGK1-tRPS3 results are skewed due to the unexpected low abundance of mod_NNN constructs in the RNA-Seq results, which all other constructs are normalised to.

Poly(A) sites can be changed by motif insertion (Figure \@ref(fig:quantseq-polyA-site-usage)B).
We highlight the 2 major poly(A) sites for the tRPS3 constructs and 2 for the tTSA1 constructs (black vertical lines in Figure \@ref(fig:quantseq-polyA-site-usage)B) and track the cumulative fraction of reads upstream of each major site.
...
Our 5PSeq data finds no detectable changes in 5'-phosphorylated intermediates between wild-type and modified reporter constructs, and thus does not indicate detectable changes in ribosome dynamics near the 3' end of transcripts (Supplementary Figure  \@ref(fig:combined-5p-end-read-plot)).
...
Overall, poly(A) site mapping showed that poly(A) sites depended on the terminator sequence, at most weakly on the coding sequence or promoter context, and not on the plasmid compared to genomic context.
Most reporter mRNAs retained the expected poly(A) site and motifs predicted from the native terminator context, except for a new alternative poly(A) site in tRPS3 mod_NNN constructs.
This was an unexpected consequence of composing cis-regulatory elements, upon introducing "random" insertions of no known function.
