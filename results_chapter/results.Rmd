...
First, to ask if poly(A) site depends only on the terminator sequence or on the wider promoter and coding sequence context.
...
In native genes the poly(A) site locations were highly reproducible across samples, giving us confidence that changes in reporter poly(A) sites are meaningful (Supplementary Figure \@ref(fig:rnaseq-QC-QuantSeq-and-5PSeq-genomic-polyA)).
We also checked that counts of native mRNAs are highly correlated between samples, giving us confidence that changes in construct detection are meaningful (Supplementary Figure \@ref(fig:rnaseq-QC-correlation)).

Poly(A) sites are largely in the same positions relative to the stop codon in wild-type terminators at native genomic loci and plasmid constructs (Figure \@ref(fig:rnaseq-genomic-vs-plasmid-polyA)).
...
Poly(A) sites are conserved in terminators regardless of the paired promoter, with tPGK1, tRPS3, and tTOS6 expressed downstream of pRPS3, and tTSA1 expressed paired with cognate pTSA1.
...
We excluded a 420nt tPMA1 terminator from our analysis because the terminator was longer than library fragment lengths so we could not distinguish plasmid from genomic poly(A)-site reads.
...
We then mapped poly(A)-sites for three motif insertion constructs with large effect sizes in the qPCR results: mod_NAA, mod_HTH and mod_NTN, together with WT and mod_NNN controls, within three promoter-terminator contexts: pRPS3-tRPS3, pPGK1-tRPS3 and pTSA1-tTSA1.
...
However, the poly(A) mapping results confirm that at least one copy of the motifs of interest are present in every major isoform for all constructs.
