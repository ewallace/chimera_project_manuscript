...
```{r rnaseq-QC-genomic-polyA, fig.cap="\\textbf{Poly(A) site locations are highly reproducible across samples.} ", fig.align='center', out.width="98%"}
knitr::include_graphics(here("supplementary_data_chapter/figures/motif_position_histograms.png"), dpi = 300) 
```
...
```{r rnaseq-QC-QuantSeq-and-5PSeq-genomic-polyA, fig.cap="\\textbf{Poly(A) site usage for select PGK1, TSA1 and RPS3 terminators is reproducible.} Cumulative counts of reads mapped downstream of native genomic gene stop codons as fraction of total reads mapped to the terminator, coloured by sample. A, Poly(A) site usage for genomic PGK1, TSA1 and RPS3 terminators is the same across samples for both QuantSeq and 5PSeq assays. Higher variability in QuantSeq PGK1 poly(A) site usage is due to low overall transcript counts. B, Poly(A) site usage measured by 5PSeq is the same for genomic and plasmid-expressed wild-type RPS3 terminator.", fig.align='center'}
