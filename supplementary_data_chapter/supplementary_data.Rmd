...
```{r mTurq-seq}
knitr::kable(tibble("mTurquoise2 construct sequences are available on the manuscript github repo" = "https://github.com/DimmestP/chimera_project_manuscript/ tree/main/supplementary_data_chapter/data/ mTurq_fluorescence_reporter_dna_sequences.csv"), caption = "\\textbf{Table showing the DNA sequences for all mTurquoise2 reporter constructs.}", format = "latex", align = "c")  %>%
  kableExtra::kable_styling(latex_options = "hold_position", font_size = 7) %>%
  kableExtra::column_spec(1, width="30em")
```
...
# Setting `results = 'asis'` allows for using Latex within the code chunk
  ...
  kableExtra::row_spec(0, bold = TRUE))
    cat('\\end{tabular}}\\endgroup{}')
    cat("\\caption{\\textbf{Tables showing relative usage of the two major Poly(A) sites in tRPS3 and tTSA1 terminators across constructs, measured by QuantSeq. p values are calculated using a two sided Wilcoxon signed rank exact test with respect to the mod\\_NNN construct.}}")
...
# Setting `results = 'asis'` allows for using Latex within the code chunk
  ...
  kableExtra::row_spec(0, bold = TRUE))
    cat('\\end{tabular}}\\endgroup{}')
    cat("\\caption{\\textbf{Tables showing relative usage of the two major Poly(A) sites in tRPS3 and tTSA1 terminators across constructs, measured by 5PSeq. p values are calculated using a two sided Wilcoxon signed rank exact test with respect to the mod\\_NNN construct.}}")
...
```{r rnaseq-QC-QuantSeq-and-5PSeq-genomic-polyA, fig.cap="\\textbf{Poly(A) site usage for select PGK1, TSA1 and RPS3 terminators is reproducible.} Cumulative counts of reads mapped downstream of native genomic gene stop codons as fraction of total reads mapped to the terminator, coloured by sample. (\\textbf{A}) Poly(A) site usage for genomic PGK1, TSA1 and RPS3 terminators is the same across samples for both QuantSeq and 5PSeq assays. Higher variability in QuantSeq PGK1 poly(A) site usage is due to low overall transcript counts. (\\textbf{B}) Poly(A) site usage measured by 5PSeq is the same for genomic and plasmid-expressed wild-type RPS3 terminator.", fig.align='center'}
